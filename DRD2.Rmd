*Step 0:* Load all the necessary libraries
...
*Step 1:* Load the raw data using the minfi package and create an object called RGset that stores the RGChannelSet object.
...
This step involves importing the raw methylation data from the Illumina array. The RGset object contains the red and green fluorescence intensities, which are needed for subsequent data processing and normalization.

*Step 2:* Create the Red and Green dataframes to store the red and green fluorescences, respectively.
...
This step extracts the red and green channel intensities from the RGset object. These intensities represent the hybridization signals from the methylated and unmethylated probes, respectively.

*Step 3:* Extract the red and green fluorescence values for a specific address and determine the probe type and color if it's a Type I probe.
...
This step involves querying specific probe addresses to understand their fluorescence values. Type I probes have both red and green fluorescence, whereas Type II probes only have green. This information helps in understanding the hybridization patterns and probe types.

*Step 4:* Create the MSet.raw object using the minfi package.
...
The MSet.raw object contains the methylation and unmethylation signal intensities. This object is needed for quality control and subsequent analysis steps.

*Step 5:* Perform Quality Checks

QCplot: the QC plot visualizes the quality of the samples. It helps in identifying any samples with poor quality that might need to be excluded from further analysis.
...
Checking the intensity of negative controls ensures that there is no significant background signal, which could affect the accuracy of the methylation measurements.
...
Detection p-values assess the reliability of each probe's measurement. Probes with p-values above a threshold are considered unreliable and may be filtered out.
...
*Step 6*: Calculate raw beta and M values and plot the density distributions for WT and MUT groups.
...
Beta values represent the ratio of methylated signal to the total signal, while M values are logit-transformed beta values. Comparing the density plots of these values for WT and MUT groups can reveal differences in methylation patterns between the two groups.

*Step 7:* Normalize the data using the assigned normalization function and create plots to compare raw and normalized data.
...
Normalization corrects for technical variations and ensures that methylation measurements are comparable across samples. Comparing density plots and boxplots before and after normalization can show the effectiveness of the normalization process and,  ideally, normalization reduces variability and makes the distributions of beta values more uniform across samples and groups.

*Step 8:* Perform PCA on the normalized beta values and plot the results.
...
PCA reduces the dimensionality of the data and helps visualize the overall structure. It can reveal whether samples cluster by group (WT vs. MUT), sex, or batch effects
...
Statistical tests, and in this case the Mann-Whitney test, identify probes with significant differences in methylation levels between groups. The number of differentially methylated probes indicates the extent of methylation changes between groups.

*Step 10*: Apply Bonferroni and BH corrections and count significant probes.
...
Multiple testing corrections adjust for the large number of comparisons, reducing the likelihood of false positives. Bonferroni is more stringent, while BH is less conservative.
The corrected p-values show how many probes remain significant after adjusting for multiple tests.

*Step 11:* Produce a Volcano Plot and Manhattan Plot

The volcano plot displays the relationship between the fold change (delta) and the significance (p-value) of each probe. Significant probes with large fold changes are highlighted, additionally, points in the top right and left corners represent probes with significant and large methylation differences between groups.
...
The Manhattan plot shows the distribution of p-values across the genome. Each point represents a probe, with the x-axis showing its genomic position and the y-axis its -log10(p-value).
...
print(kable(head(input_Manhattan)))
...
*Step 12:* Create a heatmap of the top 100 differentially methylated probes: heatmaps visualize the methylation levels of significant probes across samples and clustering in the heatmap can reveal patterns and groupings.
Distinct clusters in the heatmap indicate consistent methylation differences between WT and MUT samples.
